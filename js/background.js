...
let settings = {
  ...
  /**
   * Filter for Hispanic percentage labels. Values can be:
   *   'lt10' – show labels only for counties where the Hispanic population
   *           percentage is less than 10%;
   *   'lt25' – show labels only where the percentage is less than 25%;
   *   'lt50' – show labels only where the percentage is less than 50%;
   *   'all'  – show labels for all counties regardless of percentage.
   * When undefined or unknown, defaults to 'all'.  The popup UI allows
   * users to change this filter and the content script interprets the
   * value accordingly when displaying labels.
   */
  hispanicFilter: 'all'
};
